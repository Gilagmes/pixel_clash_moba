...
let mode = "3v3", hero = "warrior", ws = null, me = null, players = [], minions = [], towers = [], bases = [], reconnectTimer = null, finished = false;
...
document.getElementById("play").addEventListener("click", () => { menu.classList.add("hidden"); game.classList.remove("hidden"); resultEl.classList.add("hidden"); finished = false; modeLabel.textContent = mode === "5v5" ? "5 × 5" : "3 × 3"; connect(); });
...
function connect() {
  ...
  ws.onmessage = event => {
    try {
      ...
      if (m.type === "combat") { statusEl.textContent = m.action === "skill" ? `⚡ Q: ${m.hits} попад.` : "⚔️ Удар!"; setTimeout(updateStatus, 650); }
      if (m.type === "victory") { finished = true; const won = Number(m.winner) === Number(me?.team); resultTitle.textContent = won ? "🏆 ПОБЕДА!" : "💀 ПОРАЖЕНИЕ"; resultText.textContent = won ? "Вражеская база уничтожена" : "Ваша база уничтожена"; resultEl.classList.remove("hidden"); }
      if (m.type !== "state") return;
      players = Array.isArray(m.players) ? m.players : []; minions = Array.isArray(m.minions) ? m.minions : []; towers = Array.isArray(m.towers) ? m.towers : []; bases = Array.isArray(m.bases) ? m.bases : [];
      if (me) me = players.find(p => p.id === me.id) || null;
      if (!me) me = players[players.length - 1] || null;
      updateStatus(); draw();
    } catch { statusEl.textContent = "Ошибка данных"; }
  };
  ws.onerror = () => { if (!finished) statusEl.textContent = "Ошибка соединения"; };
  ws.onclose = () => { ws = null; if (!game.classList.contains("hidden") && !finished) { statusEl.textContent = "Переподключение..."; clearTimeout(reconnectTimer); reconnectTimer = setTimeout(connect, 1500); } };
}
function updateStatus() {
  if (!me) return;
  const max = mode === "5v5" ? 10 : 6;
  const hp = Math.max(0, Math.round(me.hp ?? 0));
  const gold = Math.max(0, Math.round(me.gold ?? 0));
  document.getElementById("coins").textContent = gold;
  statusEl.textContent = `Игроков: ${players.length}/${max} • ${me.heroName || hero} • HP ${hp} • 🪙 ${gold}`;
  deathEl.classList.toggle("hidden", me.alive !== false);
}
function move(dx, dy) { if (ws?.readyState !== WebSocket.OPEN || !me?.alive || finished) return; ws.send(JSON.stringify({ type: "move", x: me.x + dx, y: me.y + dy })); }
function attack() { if (ws?.readyState === WebSocket.OPEN && me?.alive && !finished) ws.send(JSON.stringify({ type: "attack" })); }
function useSkill() { if (ws?.readyState === WebSocket.OPEN && me?.alive && !finished) ws.send(JSON.stringify({ type: "skill" })); }
...
function draw() {
  ...
  LANES_DRAW.forEach(y=>{ctx.fillStyle="#8d815c";ctx.fillRect(85,y-32,830,64);ctx.fillStyle="#b5a77a";ctx.fillRect(85,y-2,830,4);});
