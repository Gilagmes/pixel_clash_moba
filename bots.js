...
function chooseLane(room,b){const alive=room.towers.filter(t=>t.alive&&t.team!==b.team);if(!alive.length)return b.botLane||LANES[0];let best=LANES[0],score=-Infinity;for(const lane of LANES){const enemy=alive.filter(t=>t.laneY===lane);const friendly=room.towers.filter(t=>t.alive&&t.team===b.team&&t.laneY===lane);const enemyHp=enemy.reduce((s,t)=>s+t.hp,0),friendlyHp=friendly.reduce((s,t)=>s+t.hp,0);const pressure=(friendlyHp-enemyHp*.7);const nearbyMinions=room.minions.filter(m=>m.hp>0&&m.team===b.team&&m.laneY===lane).length;const enemyMinions=room.minions.filter(m=>m.hp>0&&m.team!==b.team&&m.laneY===lane).length;const laneScore=pressure+nearbyMinions*8-enemyMinions*3;if(laneScore>score){score=laneScore;best=lane}}return best}
function ensureBots(room){
  ...
  while(bots.length<wanted){
    ...
    const bot={id:`bot-${Math.random().toString(36).slice(2,9)}`,name:BOT_NAMES[bots.length%BOT_NAMES.length],hero,heroName:hero[0].toUpperCase()+hero.slice(1),team,x:team===1?115:885,y:lane,hp:s.maxHp,maxHp:s.maxHp,alive:true,respawnAt:0,gold:500,kills:0,deaths:0,damageBonus:0,speedBonus:0,inventory:[],level:1,xp:0,skillPoints:0,skillLevels:{q:1,w:1,e:1,r:1},isBot:true,botLane:lane,botTargetId:null,botAttackAt:0,botSkillAt:{q:0,w:0,e:0,r:0},botBuyAt:Date.now()+4000,botLaneAt:Date.now()+6000,damageDealt:0,towerDamage:0};
...
function tickBots(room){
  ...
  for(const b of room.players.filter(p=>p.isBot)){
    if(!b.alive){if(b.respawnAt&&now>=b.respawnAt){b.alive=true;b.hp=b.maxHp;b.x=b.team===1?115:885;b.y=b.botLane;b.respawnAt=0;b.botTargetId=null;b.botLaneAt=now+5000}continue}
    ...
    if(low){const home=b.team===1?125:875;b.x+=(home-b.x)*.1;b.y+=(lane-b.y)*.1;useSkill(room,b,"w",b)}
      ...
      const tower=room.towers.find(t=>t.alive&&t.team!==b.team&&t.laneY===lane&&Math.abs(t.x-b.x)<175);const alliedMinions=room.minions.some(m=>m.hp>0&&m.team===b.team&&m.laneY===lane&&Math.abs(m.x-(tower?.x||b.x))<100);
      if(tower&&alliedMinions&&now>=b.botAttackAt){const dealt=(b.hero==="mage"?9:7)+b.damageBonus;tower.hp=Math.max(0,tower.hp-dealt);b.towerDamage=(b.towerDamage||0)+Math.min(dealt,tower.hp+dealt);b.botAttackAt=now+800;if(tower.hp===0){tower.alive=false;for(const p of room.players)if(p.team===b.team){p.gold+=75;gainXp(p,50)}}}
    }
    ...
    const base=room.bases.find(x=>x.team!==b.team);const alliedMinionsNearBase=room.minions.some(m=>m.hp>0&&m.team===b.team&&Math.abs(m.x-base?.x||999)<120);if(base&&alliedMinionsNearBase&&Math.abs(base.x-b.x)<65&&Math.abs(base.y-b.y)<85&&!low&&now>=b.botAttackAt){const dealt=5+b.damageBonus*.5;base.hp=Math.max(0,base.hp-dealt);b.towerDamage=(b.towerDamage||0)+dealt;b.botAttackAt=now+900}
