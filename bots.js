...
function targetScore(t,b){const d=dist(b,t);let score=320-d;if(t.hp<t.maxHp*.35)score+=110;if(t.hero==="mage")score+=22;if(t.hero==="assassin")score+=14;return score}
function chooseHeroTarget(enemies,b,limit,focusId){let best=null,score=-Infinity;for(const e of enemies){if(!e.alive||dist(b,e)>limit||Math.abs(e.y-b.y)>130)continue;let s=targetScore(e,b);if(focusId&&e.id===focusId)s+=180;if(s>score){score=s;best=e}}return best}
...
let target=focused&&dist(b,focused)<330?focused:chooseHeroTarget(enemies,b,b.hero==="mage"?280:240,b.botFocusId);
...
if(wantsJungle){
  ...
  const range=b.hero==="mage"?180:b.hero==="assassin"?145:150;
  if(b.hero==="warrior"&&nearbyAllies===0&&danger&&danger.id!==target.id&&dist(b,danger)<170){b.x+=(danger.x-b.x)*.04;b.y+=(danger.y-b.y)*.04}
  else if(b.hero==="assassin"&&dist(b,target)<300&&nearbyAllies>0){const flank=b.team===1?-1:1;b.x+=(target.x-b.x)*.06+flank*1.2;b.y+=(target.y-b.y)*.08+Math.sin(now/350)*2}
