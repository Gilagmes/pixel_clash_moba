...
function buyBot(b){if(!b.alive)return;const wants=b.hero==="warrior"?["armor","blade","boots"]: ["blade","boots","armor"];for(const key of wants){const count=b.inventory.filter(x=>x===key).length,max=key==="boots"?2:3;if(count>=max)continue;const cost=COSTS[key]*(count+1);if(b.gold<cost)continue;b.gold-=cost;b.inventory.push(key);if(key==="blade")b.damageBonus+=6;if(key==="armor"){b.maxHp+=35;b.hp+=35}if(key==="boots")b.speedBonus+=.22;break}}
function chooseLane(room,b){const alive=room.towers.filter(t=>t.alive&&t.team!==b.team);if(!alive.length)return b.botLane||LANES[0];let best=LANES[0],score=-Infinity;for(const lane of LANES){const enemy=alive.filter(t=>t.laneY===lane),friendly=room.towers.filter(t=>t.alive&&t.team===b.team&&t.laneY===lane),enemyHp=enemy.reduce((s,t)=>s+t.hp,0),friendlyHp=friendly.reduce((s,t)=>s+t.hp,0),pressure=friendlyHp-enemyHp*.7,nearbyMinions=room.minions.filter(m=>m.hp>0&&m.team===b.team&&m.laneY===lane).length,enemyMinions=room.minions.filter(m=>m.hp>0&&m.team!==b.team&&m.laneY===lane).length,laneScore=pressure+nearbyMinions*8-enemyMinions*3;if(laneScore>score){score=laneScore;best=lane}}return best}
function ensureBots(room){if(!room||room.finished||!room.players.some(p=>!p.isBot))return;if(room.botSpawnAt==null)room.botSpawnAt=Date.now()+3500;if(Date.now()<room.botSpawnAt)return;const humans=room.players.filter(p=>!p.isBot).length,wanted=room.maxPlayers-humans;let bots=room.players.filter(p=>p.isBot);while(bots.length<wanted){const t1=room.players.filter(p=>p.team===1).length,t2=room.players.filter(p=>p.team===2).length,team=t1<=t2?1:2,hero=HEROES[(bots.length+team)%HEROES.length],s=STATS[hero],lane=LANES[bots.length%3],bot={id:`bot-${Math.random().toString(36).slice(2,9)}`,name:BOT_NAMES[bots.length%BOT_NAMES.length],hero,heroName:hero[0].toUpperCase()+hero.slice(1),team,x:team===1?115:885,y:lane,hp:s.maxHp,maxHp:s.maxHp,alive:true,respawnAt:0,gold:500,kills:0,deaths:0,damageBonus:0,speedBonus:0,inventory:[],level:1,xp:0,skillPoints:0,skillLevels:{q:1,w:1,e:1,r:1},isBot:true,botLane:lane,botTargetId:null,botAttackAt:0,botSkillAt:{q:0,w:0,e:0,r:0},botBuyAt:Date.now()+4000,botLaneAt:Date.now()+6000,botJungleAt:Date.now()+9000,damageDealt:0,towerDamage:0,buffs:[],buffUntil:0};room.players.push(bot);bots.push(bot)}}
...
if(!b.alive){if(b.respawnAt&&now>=b.respawnAt){b.alive=true;b.hp=b.maxHp;b.x=b.team===1?115:885;b.y=b.botLane;b.respawnAt=0;b.botTargetId=null;b.botLaneAt=now+5000;b.botJungleAt=now+5000}continue}
if(now>=b.botBuyAt){buyBot(b);b.botBuyAt=now+3500}
if(now>=b.botLaneAt){b.botLane=chooseLane(room,b);b.botLaneAt=now+6000}
const lane=b.botLane||LANES[0],dir=b.team===1?1:-1,enemies=room.players.filter(p=>p.alive&&!p.isBot&&p.team!==b.team),allies=room.players.filter(p=>p.alive&&p.team===b.team&&p.id!==b.id),low=b.hp<b.maxHp*.32,danger=nearest(enemies,b,190,p=>Math.abs(p.y-b.y)<100);
...
if(wantsJungle){
  ...
  else if(now>=b.botAttackAt){
    b.botAttackAt=now+(b.hero==="assassin"?500:700);
    const dmg=(STATS[b.hero].damage+b.damageBonus)*(1+(b.damageBuff||0));
    const dealt=Math.min(camp.hp,dmg);camp.hp=Math.max(0,camp.hp-dmg);b.damageDealt=(b.damageDealt||0)+dealt;
    if(camp.hp<=0){
      camp.alive=false;camp.respawnAt=Date.now()+camp.respawn;
      b.gold+=(camp.type==="boss"?250:camp.type==="red"?90:75);
      gainXp(b,camp.type==="boss"?180:60);
      b.buffs=b.buffs||[];b.buffs=[...new Set([...b.buffs,camp.buff])];b.buffUntil=Date.now()+(camp.type==="boss"?60000:30000);
      if(camp.buff==="red")b.damageBuff=.18;
      if(camp.buff==="dragon")b.damageBuff=.3;
      if(camp.buff==="blue")b.cooldownBuff=.25;
    }
  }
  b.botJungleAt=now+3000;
}else if(low){
  const home=b.team===1?125:875;b.x+=(home-b.x)*.1;b.y+=(lane-b.y)*.1;useSkill(room,b,"w",b)
}else if(target){
  b.botTargetId=target.id;const range=b.hero==="mage"?180:b.hero==="assassin"?145:150;
  if(dist(b,target)>range*.75){b.x+=Math.sign(target.x-b.x)*(1.1+(b.speedBonus||0));b.y+=(target.y-b.y)*.06}
  else if(now>=b.botAttackAt){b.botAttackAt=now+(b.hero==="assassin"?500:700);damage(room,b,target,(STATS[b.hero].damage+b.damageBonus)*(1+(b.damageBuff||0)))}
  ...
  b.botTargetId=null;
  const ally=nearest(allies,b,180,p=>p.hp<p.maxHp*.55);
  if(ally&&b.hero==="warrior"){b.x+=(ally.x-b.x)*.025;b.y+=(ally.y-b.y)*.025}
...
const alliedMinionsNearBase=room.minions.some(m=>m.hp>0&&m.team===b.team&&Math.abs(m.x-(base?.x||999))<120);
if(base&&alliedMinionsNearBase&&Math.abs(base.x-b.x)<65&&Math.abs(base.y-b.y)<85&&!low&&now>=b.botAttackAt){const dealt=(5+b.damageBonus*.5)*(1+(b.damageBuff||0));const actual=Math.min(base.hp,dealt);base.hp=Math.max(0,base.hp-dealt);b.towerDamage=(b.towerDamage||0)+actual;b.botAttackAt=now+900}
