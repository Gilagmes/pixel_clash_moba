...
function useSkill(room,b,key,target){const now=Date.now(),baseCd={q:3500,w:6000,e:8000,r:20000}[key],cd=Math.round(baseCd*(1-(b.cooldownReduction||0)));if(now<b.botSkillAt[key])return false;b.botSkillAt[key]=now+cd;const rank=b.skillLevels[key]||1;if(key==="w"){const heal=(b.hero==="warrior"?42:28)+Math.round(b.maxHp*(b.hero==="warrior"?.18:.12))+rank*4;b.hp=Math.min(b.maxHp,b.hp+heal);return true}if(!target||target.alive===false){b.botSkillAt[key]=now;return false}const base=b.hero==="mage"?(key==="r"?110:46):b.hero==="assassin"?(key==="r"?125:key==="q"?58:38):(key==="r"?85:key==="q"?34:28),range=key==="e"?125:key==="r"?240:(b.hero==="mage"?280:190);if(dist(b,target)>range)return false;if(key==="e"){const dir=b.team===1?1:-1;b.x=clamp(target.x-dir*35,80,920);b.y=clamp(target.y,90,810)}let value=base+b.damageBonus+rank*(key==="r"?5:key==="q"?3:2);if(b.hero==="mage")value*=1.06;if(b.hero==="assassin"&&target.hp<target.maxHp*.35)value*=1.18;return damage(room,b,target,value)}
function targetScore(t,b){const d=dist(b,t);let score=320-d;if(t.hp<t.maxHp*.35)score+=b.hero==="assassin"?190:110;if(t.hero==="mage")score+=b.hero==="assassin"?35:22;if(t.hero==="assassin")score+=b.hero==="warrior"?28:14;return score}
...
function chooseTeamFocus(room,b,enemies){const candidates=enemies.filter(e=>e.alive&&dist(b,e)<380);if(!candidates.length)return null;let best=null,score=-Infinity;for(const e of candidates){const alliesNear=room.players.filter(p=>p.alive&&p.team===b.team&&p.id!==b.id&&dist(p,e)<260).length;const enemyNear=room.players.filter(p=>p.alive&&p.team!==b.team&&dist(p,e)<160).length;let s=targetScore(e,b)+alliesNear*75-enemyNear*18;if(b.hero==="assassin"&&e.hp<e.maxHp*.45)s+=90;if(b.hero==="warrior"&&e.hero==="assassin")s+=35;if(s>score){score=s;best=e}}return best?.id||null}
function teamCenter(room,b){const a=room.players.filter(p=>p.alive&&p.team===b.team);if(!a.length)return b;return{x:a.reduce((s,p)=>s+p.x,0)/a.length,y:a.reduce((s,p)=>s+p.y,0)/a.length}}
function teamThreat(room,b){const enemies=room.players.filter(p=>p.alive&&p.team!==b.team);let best=null,score=-Infinity;for(const e of enemies){const allies=room.players.filter(p=>p.alive&&p.team===b.team&&p.id!==b.id&&dist(p,e)<240).length;const near=room.players.filter(p=>p.alive&&p.team!==b.team&&dist(p,e)<170).length;const s=allies*80-near*25+(e.maxHp-e.hp)/Math.max(1,e.maxHp)*100;if(s>score){score=s;best=e}}return best}
...
const center=teamCenter(room,b),threat=teamThreat(room,b);
...
let target=focused&&dist(b,focused)<350?focused:chooseHeroTarget(enemies,b,b.hero==="mage"?300:260,b.botFocusId);
...
if(wantsJungle){
  ...
  if(b.hero==="warrior"&&danger&&dist(b,danger)<220){b.x+=(danger.x-b.x)*.025;b.y+=(danger.y-b.y)*.025}else{b.x+=(home-b.x)*.1;b.y+=(lane-b.y)*.1}
  if(now>=b.botSkillAt.w)useSkill(room,b,"w",b)
}else if(target){
  ...
  if(b.hero==="warrior"){
    if(nearbyAllies===0&&danger&&danger.id!==target.id&&dist(b,danger)<190){b.x+=(danger.x-b.x)*.055;b.y+=(danger.y-b.y)*.055}
    else if(nearbyAllies>0&&dist(b,target)>range*.7){b.x+=(target.x-b.x)*.055;b.y+=(target.y-b.y)*.055}
  }else if(b.hero==="mage"){
    if(dist(b,target)<125){b.x-=(target.x-b.x)*.08;b.y+=(b.y-target.y)*.05}else if(dist(b,target)>range*.72){b.x+=Math.sign(target.x-b.x)*(1+(b.speedBonus||0));b.y+=(target.y-b.y)*.04}
  ...
  const regroup=allies.filter(a=>dist(a,center)<120).length;
  if(ally&&b.hero==="warrior"){b.x+=(ally.x-b.x)*.035;b.y+=(ally.y-b.y)*.035}
  else if(regroup===0&&allies.length){b.x+=(center.x-b.x)*.025;b.y+=(center.y-b.y)*.025}
