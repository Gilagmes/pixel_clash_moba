...
function targetScore(t,b){const d=dist(b,t);let score=300-d;if(t.hp<t.maxHp*.35)score+=100;if(t.hero==="mage")score+=18;if(t.hero==="assassin")score+=12;return score}
function chooseHeroTarget(enemies,b,limit){let best=null,score=-Infinity;for(const e of enemies){if(!e.alive||dist(b,e)>limit||Math.abs(e.y-b.y)>120)continue;const s=targetScore(e,b);if(s>score){score=s;best=e}}return best}
...
const low=b.hp<b.maxHp*.32;
const danger=chooseHeroTarget(enemies,b,190);
let target=chooseHeroTarget(enemies,b,b.hero==="mage"?280:240);
...
if(wantsJungle){
  ...
  b.botTargetId=null;
  const ally=nearest(allies,b,220,p=>p.hp<p.maxHp*.55);
